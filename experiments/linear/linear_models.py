...
try:
    with open(os.path.join(experiment_directory, "parameters_linear.json"), "r") as f:
        ...
        with open(os.path.join(root, "parameters_linear.json"), "r") as f:
            params = json.load(f)
...
def generate_nonstationary_trend_linear():
    ...
    time_series = np.sin(theta(np.arange(params["length"])) + x0) * np.sqrt(2)
    obs_noise = rand.normal(0, model_params["obs_noise"], params["length"])
    trend = model_params["trend"] * np.exp(np.linspace(0,1,num=params["length"]))
    return time_series * trend + obs_noise
"""

def generate_stationary_variance_increase_linear():
